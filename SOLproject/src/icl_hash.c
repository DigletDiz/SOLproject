...
void *
icl_hash_wfind(icl_hash_t *ht, void* key, size_t* size)
{
    icl_entry_t* curr;
    unsigned int hash_val;
...
int icl_hash_delete(icl_hash_t *ht, void* key, void (*free_key)(void*), void (*free_data)(void*))
{
    ...
    for (curr=ht->buckets[hash_val]; curr != NULL; )  {
        if ( ht->hash_key_compare(curr->key, key)) {
            ...
            if (*free_key && curr->key) {(*free_key)(curr->key); curr->key = NULL;}
            if (*free_data && curr->data) {(*free_data)(curr->data); curr->data = NULL;}
            ht->nentries--;
            free(curr);
            curr = NULL;
            return 0;
